...
function presentReady(key){
 ...
 const draw=()=>{
  ...
  drawFrame(frame,request.overlay);
  ...
  if(quality&&!gpuPresenter&&presentation.pixelChecks.length<2&&presentation.drawn%60===0){
...
function presentSelected(){
 ...
 if(engine._web_selected_redraw()&&heldFrame&&Math.round(heldFrame.timestamp)===key){drawFrame(heldFrame,overlay);presentation.redraws++;engine._web_presented();return;}
 if(key<0)return;
...
self.onmessage = async ({data}) => {
  try {
    if (data.type === 'init') {
      ...
      canvas = data.canvas;
      gpuPresenter=await ExternalFramePresenter.create(canvas,message=>{pumpFailed=true;clearTimeout(timer);cleanupFrames();post({type:'error',message});});
      ...
      gpuPresenter?.destroy();gpuPresenter=null;
      engine?._web_destroy();
...
self.inspectRetainedColor=async()=>{
 ...
 return {metadata:{colorSpace:heldFrame.colorSpace.toJSON(),format:heldFrame.format,visibleRect:heldFrame.visibleRect.toJSON(),displayWidth:heldFrame.displayWidth,displayHeight:heldFrame.displayHeight,track:videoTrack},reference:await encode(reference),candidate:await encode(canvas)};
};
