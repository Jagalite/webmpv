...
function clear(){generation++;if(decoder&&decoder.state!=='closed')decoder.close();decoder=null;for(const frame of queue)closeFrame(frame);for(const frame of copying)closeFrame(frame);queue=[];draining=flushed=false;submitted=consumed=0;failure=null;publishHint();}
function configure(){
 if(copiesInFlight)throw Error('Previous copy still pending; use software');
 const current=generation;
 decoder=new VideoDecoder({error:error=>{if(current===generation){failure=String(error);stats.errors++;publishHint();postMessage({wakeup:true});}},output:frame=>{
  stats.receivedFrames++;if(stats.receivedFrames>48){closeFrame(frame);return;}
  if(current!==generation){closeFrame(frame);return;}
  if(queue.length>=8){closeFrame(frame);failure='Frame queue limit';stats.errors++;publishHint();postMessage({wakeup:true});return;}
  queue.push(frame);publishHint();postMessage({wakeup:true});stats.peakFrames=Math.max(stats.peakFrames,queue.length);lastProgress=performance.now();
 }});
 decoder.configure(configuration);lastProgress=performance.now();publishHint();
}
self.onmessage=({data})=>{
 if(data.type==='cancel'){
  disabled=true;clear();
 ...
 Atomics.store(header,13,0);Atomics.store(header,14,1);
 ...
 try{
  if(operation===1){
   ...
   if(operation===2){
    ...
    decoder.flush().then(()=>{if(epoch===generation){flushed=true;publishHint();postMessage({wakeup:true});}},error=>{if(epoch===generation){failure=String(error);publishHint();postMessage({wakeup:true});}});
   }else if(operation===4){
