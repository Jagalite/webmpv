...
const result={started:new Date().toISOString(),browser:browser.version(),tests:[],passed:false};
...
async function open(name,id){await page.evaluate(async({name,id})=>{await createPlayer();player.resize(1920,1080);await player.openRemote({url:`http://127.0.0.1:4180/media/${name}?id=${id}`});await player.play();},{name,id});await wait(()=>player.diagnostics?.rendered>5&&player.audioDiagnostics().mediaFrames>4096);}
async function destroy(){await page.evaluate(()=>player?.destroy());for(let i=0;i<30&&page.workers().length;i++)await page.waitForTimeout(100);assert.equal(page.workers().length,0);}
try{
 ...
 await check('real browser CORS, authorization, renewal, cookies and rejected responses',async()=>{
  ...
  for(const mode of ['auth','cookie','cors-denied','forbidden','ignore-range','bad-range','encoded','redirect','retry','truncate']){
   const id=`m2-${mode}-${Date.now()}`;await control(id,{mode});
 ...
 for(const name of ['front','tail'])await check(`${name} index: shaped startup and distant seek`,async()=>{
  ...
  await page.evaluate(async({name,id})=>{await createPlayer();player.resize(1920,1080);window.start=performance.now();window.initialRendered=player.diagnostics?.rendered||0;await player.openRemote({url:`http://127.0.0.1:4180/media/${name}?id=${id}`});await player.play();},{name,id});
  await wait(()=>player.diagnostics?.rendered>initialRendered+1&&player.audioDiagnostics().mediaFrames>128);
  ...
  await destroy();console.log('shaped',name,JSON.stringify({startup,seek}));
 ...
 await check('multiple tracks, volume, rate and composed ASS paused redraw',async()=>{
  ...
  await page.screenshot({path:'results/m2/ass-composition.png'});await page.evaluate(()=>player.resize(1280,720));await page.waitForTimeout(300);await page.screenshot({path:'results/m2/ass-resize.png'});await destroy();return {tracks,compare};
 });
 await check('VFR and B-frame playback reaches the final decoded frames',async()=>{await open('vfr','m2-vfr');await wait(()=>player.properties.get('eof-reached')===true,20000);const evidence=await page.evaluate(()=>({video:player.diagnostics,position:player.properties.get('time-pos'),errors:playerErrors}));assert.ok(evidence.position>11.8);assert.deepEqual(evidence.errors,[]);await destroy();return evidence;});
