...
const result={started:new Date().toISOString(),durationSeconds:duration,artifactHashes:hashes,passed:false};
...
try{
 ...
 for(let i=0;Date.now()-start<duration*1000;i++){
  ...
  sample.elapsedSeconds=(Date.now()-start)/1000;sample.processRSS=processRSS;sample.totalRSS=processRSS.reduce((n,p)=>n+p.bytes,0);samples.push(sample);
...
finally{result.finished=new Date().toISOString();await writeFile('results/m2/long.json',JSON.stringify(result,null,2)+'\n');await browser.close();}
