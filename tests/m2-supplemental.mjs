...
const logs=[],result={started:new Date().toISOString(),tests:[],passed:false};page.on('console',m=>logs.push(m.text()));
...
try{
 ...
 await check('attached ASS shaping, karaoke, vectors and overlap match native reference at DPR 2',async()=>{
  ...
  const comparison=await page.evaluate(async({on,off,refOn,refOff})=>{
   ...
   const [a,b,c,d]=await Promise.all([on,off,refOn,refOff].map(pixels));const w=1920,h=1080;
   ...
   return [[0,0,1920,300,'karaoke'],[700,400,1250,700,'vectors and overlap'],[0,850,1920,1080,'shaping and accents']].map(([x0,y0,x1,y1,name])=>{let n=0,r=0,matched=0,covered=0;for(let y=y0;y<y1;y++)for(let x=x0;x<x1;x++){if(actual[y*w+x]){n++;if(near(reference,x,y))matched++;}if(reference[y*w+x]){r++;if(near(actual,x,y))covered++;}}return {name,pixels:n,referencePixels:r,precision:matched/n,recall:covered/r};});
  },{on,off,refOn,refOff});
  ...
  await page.evaluate(()=>player.subtitleVisible(true));await page.waitForTimeout(300);await page.evaluate(()=>player.resize(1280,720));await page.waitForTimeout(300);const resized=await capture('results/m2/ass-dpr2-resized.png');assert.ok(resized.length>1000);await page.evaluate(()=>player.destroy());return {comparison,deviceScaleFactor:2,tolerance:'RGB delta >=12; 3-pixel edge tolerance; each region precision and recall >=95%'};
 });
 ...
 await check('late authorization renewal cannot satisfy a replacement source',async()=>{
  for(const id of ['m2-auth-old','m2-auth-new'])await fetch(`http://127.0.0.1:4180/control?id=${id}`,{method:'POST',body:JSON.stringify({mode:'auth'})});
