...
self.onmessage=async({data})=>{
 try{
  ...
  if(data.type==='init'){
   engine=await createRemux({printErr:message=>postMessage({type:'log',message})});engine.io=data.mailbox;engine.raps=[];engine.tracks=[];
   ...
   postMessage({type:'ready',duration,mime,tracks:engine.tracks,buffer,stats:{...stats}},[buffer]);
