...
export class RangeReader {
  ...
  async fetchBlock(start,epoch){
    ...
    for(let attempt=0;attempt<10;attempt++){
      if(this.closed||epoch!==this.epoch)throw new DOMException('Superseded','AbortError');
      ...
      try{
        ...
        response=await fetch(this.options.url,{headers,credentials:this.options.credentials,redirect:'error',cache:'no-store',signal:controller.signal});
        if(response.status===401&&!refreshed&&this.refresh){
          await response.body?.cancel();clearTimeout(timer);const update=await this.refresh();
        ...
        if(response.status===416){
          ...
          if(match&&this.total!==undefined&&BigInt(match[1])===this.total&&offset>=this.total){await response.body?.cancel();return buffer.subarray(0,received);}
        ...
        while(true){touch();const {done,value}=await reader.read();if(done)break;
        ...
        controller.abort();await reader?.cancel().catch(()=>{});await response?.body?.cancel().catch(()=>{});
        if(epoch!==this.epoch||this.closed)throw new DOMException('Superseded','AbortError');
        ...
        await new Promise(resolve=>{const timer=setTimeout(done,wait);const self=this;function done(){clearTimeout(timer);self.retryWake=null;resolve();}this.retryWake=done;});
