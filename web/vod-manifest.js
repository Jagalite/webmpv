...
export function validateVODManifest(bytes,format){
  const text=new TextDecoder('utf-8',{fatal:true}).decode(bytes).replace(/^\uFEFF/,'');
  if(format==='hls'){
    const lines=text.split(/\r?\n/).map(s=>s.trim());
    if(lines[0]!=='#EXTM3U')throw Error('Invalid HLS manifest');
    if(lines.some(s=>/^#EXT-X-(KEY|SESSION-KEY|PART|PRELOAD-HINT|SERVER-CONTROL|RENDITION-REPORT|SKIP):/.test(s)))throw Error('Encrypted or live HLS is unsupported');
    const variants=lines.filter(s=>s.startsWith('#EXT-X-STREAM-INF:')).length;
    if(variants>1||lines.some(s=>s.startsWith('#EXT-X-I-FRAME-STREAM-INF:')))throw Error('Adaptive HLS is unsupported');
    if(!variants&&!lines.includes('#EXT-X-ENDLIST'))throw Error('HLS must be finite VOD');
  ...
  for(const match of clean.matchAll(/<([^>]+)>/g)){
    ...
    if(!root){if(name!=='MPD'||!/(?:^|\s)type\s*=\s*(['"])static\1(?:\s|$)/.test(token))throw Error('DASH must be static VOD');root=true;}
    if(['ContentProtection','UTCTiming','Location','PatchLocation','EventStream'].includes(name)||/\bxlink:/.test(token))throw Error('Unsupported DASH extension');
    ...
    if(name==='Representation'){
      const parent=stack.at(-1);if(parent?.name!=='AdaptationSet'||++parent.representations>1)throw Error('Adaptive DASH is unsupported');
    }
  ...
  if(hasTemplate&&!hasTimeline&&!hasDuration)throw Error('DASH template requires a finite duration or timeline');
